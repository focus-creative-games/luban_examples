...
bool test::TestScriptableObject::deserialize(::luban::ByteBuf& _buf)
{
    ...
    if(!vector2::deserializevector2(_buf, v2)) return false;
    if(!vector3::deserializevector3(_buf, v3)) return false;
    if(!vector4::deserializevector4(_buf, v4)) return false;
...
bool DefineFromExcel2::deserialize(::luban::ByteBuf& _buf)
{
    ...
    {::luban::int32 n; if(!_buf.readSize(n)) return false; n = std::min(n, ::luban::int32(_buf.size())); k10.reserve(n);for(int i = 0 ; i < n ; i++) { ::luban::SharedPtr<vector3> _e;  if(!vector3::deserializevector3(_buf, _e)) return false; k10.push_back(_e);}}
    {::luban::int32 n; if(!_buf.readSize(n)) return false; n = std::min(n, ::luban::int32(_buf.size())); k11.reserve(n);for(int i = 0 ; i < n ; i++) { ::luban::SharedPtr<vector4> _e;  if(!vector4::deserializevector4(_buf, _e)) return false; k11.push_back(_e);}}
