...
pub struct DemoType2 {
    ...
    pub x14: std::sync::Arc<DemoDynamic>,
    pub s1: String,
    pub t1: u64,
    pub k1: Box<[i32]>,
    pub k2: Vec<i32>,
    ...
    pub k15: Box<[std::sync::Arc<DemoDynamic>]>,
}
...
pub enum DemoDynamic {
    DemoD2(DemoD2),
    DemoE1(DemoE1),
    RoleInfo(crate::test::login::RoleInfo),
    DemoD5(DemoD5),
}
impl DemoDynamic {
    fn get_x1(&self) -> &i32 {
    ...
    pub fn new(json: &serde_json::Value) -> Result<std::sync::Arc<DemoDynamic>, LubanError> {
...
impl DemoD2 {
    pub fn new(json: &serde_json::Value) -> Result<DemoDynamic, LubanError> {
        ...
        Ok(DemoDynamic::DemoD2(DemoD2 { x1, x2 }))
    }
}
...
impl DemoE1 {
    pub fn new(json: &serde_json::Value) -> Result<DemoDynamic, LubanError> {
        ...
        Ok(DemoDynamic::DemoE1(DemoE1 { x1, x3, x4 }))
    }
}
...
impl DemoD5 {
    pub fn new(json: &serde_json::Value) -> Result<DemoDynamic, LubanError> {
        ...
        Ok(DemoDynamic::DemoD5(DemoD5 { x1, time }))
    }
}
...
impl MultiRowRecord {
    pub fn new(json: &serde_json::Value) -> Result<MultiRowRecord, LubanError> {
        ...
        let multi_rows4 = std::collections::HashMap::from_iter(json["multi_rows4"].as_array().unwrap().iter().map(|x| {
            let array = x.as_array().unwrap();
            ((array[0].as_i64().unwrap() as i32), crate::test::MultiRowType2::new(&array[1]).unwrap())
...
pub struct TestNull {
    ...
    pub x4: Option<std::sync::Arc<DemoDynamic>>,
...
impl TestNull {
    pub fn new(json: &serde_json::Value) -> Result<TestNull, LubanError> {
        ...
        let mut x2 = None;
        if let Some(value) = json.get("x2") { x2 = Some(json["x2"].as_i64().unwrap().into()); }
        let mut x3 = None;
...
pub struct TestRef {
    ...
    pub f2: i64,
    pub f3: String,
    pub s1: std::sync::Arc<RefDynamicBase>,
}
...
impl RefDynamicBase {
    pub fn new(json: &serde_json::Value) -> Result<std::sync::Arc<RefDynamicBase>, LubanError> {
        let type_id = json["$type"].as_str().unwrap();
        match type_id {
    ...
    fn get_x(&self) -> &i32 {
        match self { RefDynamicBase::RefBean(x) => { &x.x } }
    }
    fn get_x_ref(&self) -> &Option<Arc<test::TestBeRef>> {
        match self { RefDynamicBase::RefBean(x) => { &x.x_ref } }
    }

    pub fn resolve_ref(&mut self, tables: &Tables) {
        match self {
            RefDynamicBase::RefBean(ref mut x) => { x.resolve_ref(tables) }
        }
    }
}
...
pub struct RefBean {
    pub x: i32,
    pub x_ref: Option<Arc<test::TestBeRef>>,
    pub arr: Vec<i32>,
}

impl RefBean {
    pub fn new(json: &serde_json::Value) -> Result<RefDynamicBase, LubanError> {
        let x = (json["x"].as_i64().unwrap() as i32);
        let x_ref = None;
        let arr = json["arr"].as_array().unwrap().iter().map(|field| (field.as_i64().unwrap() as i32)).collect();

        Ok(RefDynamicBase::RefBean(RefBean { x, x_ref, arr }))
    }
...
impl TestMap {
    pub fn new(json: &serde_json::Value) -> Result<TestMap, LubanError> {
        ...
        let x4 = std::collections::HashMap::from_iter(json["x4"].as_array().unwrap().iter().map(|x| {
            let array = x.as_array().unwrap();
            (array[0].as_i64().unwrap().into(), (array[1].as_i64().unwrap() as i32))
...
impl TbSingleton {
    ...
    unsafe fn resolve_ref(&mut self, tables: &Tables) {
...
impl TbTestRef {
    ...
    pub(crate) unsafe fn resolve_ref(&mut self, tables: &Tables) {
        self.data_list.iter_mut().for_each(|mut x| {
            let mut b = Box::from_raw(x.s1.as_ref() as *const crate::test::RefDynamicBase as *mut RefDynamicBase);
