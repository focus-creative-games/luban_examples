...
ByteBuf = Bright.Serialization.ByteBuf
...
function Load(typeDefs, configFileloader)
    ...
    local configPath = UnityEngine.Application.dataPath .. "/../config_data/"
...
function C.Start()
    local cfgTypeDefs = require("Gen.Types").InitTypes(byteBufFuns)
    Load(cfgTypeDefs)
end
