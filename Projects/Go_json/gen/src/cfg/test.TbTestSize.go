...
func NewTest_TbTestSize(_buf []map[string]interface{}) (*Test_TbTestSize, error) {
	_dataList := make([]*Test_TestSize, 0, len(_buf))
	dataMap := make(map[int32]*Test_TestSize)
	for _, _ele_ := range _buf {
		if _v, err2 := NewTest_TestSize(_ele_); err2 != nil {
			return nil, err2
		} else {
