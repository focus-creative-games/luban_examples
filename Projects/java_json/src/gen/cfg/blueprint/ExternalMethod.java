
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
package cfg.blueprint;

import com.google.gson.JsonElement;
import com.google.gson.JsonObject;



public final class ExternalMethod extends cfg.blueprint.Method {
    public ExternalMethod(JsonObject __json__) { 
        super(__json__);
    }

    public ExternalMethod(String name, String desc, boolean is_static, String return_type, java.util.ArrayList<cfg.blueprint.ParamInfo> parameters ) {
        super(name, desc, is_static, return_type, parameters);
    }

    public static ExternalMethod deserializeExternalMethod(JsonObject __json__) {
        return new ExternalMethod(__json__);
    }


    public static final int ID = 1739079015;

    @Override
    public int getTypeId() { return ID; }

    @Override
    public void resolve(java.util.HashMap<String, Object> _tables) {
        super.resolve(_tables);
    }

    @Override
    public String toString() {
        return "{ "
        + "name:" + name + ","
        + "desc:" + desc + ","
        + "isStatic:" + isStatic + ","
        + "returnType:" + returnType + ","
        + "parameters:" + parameters + ","
        + "}";
    }
}
